...
using OyuncuSepeti.API.Dtos;
using OyuncuSepeti.API.Helpers;
using OyuncuSepeti.API.Models;

namespace OyuncuSepeti.API.Controllers
{
    [Authorize]
    ...
    public class PhotosController : ControllerBase
    {
        ...
        public PhotosController(IOyuncuRepository repo, IMapper mapper,
        ...
        [FromForm]PhotoForCreationDto photoForCretionDto)
        {
        ...
        if(userId != int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value)) 
               ...
               return BadRequest("Zaten Profil Fotoğrafı");
               ...
               return BadRequest("Profil Fotoğrafı Olarak Seçilemedi");
        ...
        public async Task<IActionResult> DeletePhoto(int userId,int id)
        {
            if(userId != int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value)) 
               ...
               var photoFromRepo = await _repo.GetPhoto(id);
               if (photoFromRepo.IsMain)
               return BadRequest("Ana Fotoğrafı Silemezsiniz");
               ...
               if (await _repo.SaveAll())
               return Ok();
                return BadRequest("Fotoğraf Silinirken hata oluştu");
